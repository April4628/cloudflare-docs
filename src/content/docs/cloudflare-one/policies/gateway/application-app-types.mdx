...
When you use the Application selector in Gateway policies, actions taken by Gateway will be limited to the specific application defined. Gateway may also log other applications that use the same hostnames, but it will not take action unless the application was matched by the policy. This is most common for vendors with many applications, such as Google or Meta. For example, both the Facebook and Facebook Messenger apps use the `chat-e2ee.facebook.com` hostname. When evaluating traffic to the Facebook Messenger app, Gateway will only take action on Facebook Messenger traffic but log both the Facebook and Facebook Messenger apps.
